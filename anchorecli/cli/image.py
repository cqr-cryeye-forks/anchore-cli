...
@click.option(
    ...
    type=float,
    default=-1.0,
    help="Time to wait, in seconds.  If < 0, wait forever, if 0, do not wait (default=-1)",
)
@click.option(
    "--interval",
    type=float,
...
def wait(input_image, timeout, interval):
    ...
    try:
        ...
        while timeout < 0 or time.time() - t1 < timeout:
            ...
            if itype == "tag":
                ret = anchorecli.clients.apiexternal.get_image(
                    config, tag=image, history=False
                )
            elif itype == "imageid":
                ret = anchorecli.clients.apiexternal.get_image(
                    config, image_id=image, history=False
                )
            elif itype == "imageDigest":
                ret = anchorecli.clients.apiexternal.get_image(
                    config, imageDigest=image, history=False
                )
            else:
                ecode = 1
                raise Exception(
            ...
            if ret["payload"] and ret["payload"][0]["analysis_status"] in [
                "analyzed",
                "analysis_failed",
            ]:
                break
            ...
            if timeout > 0:
                print(
                    "Waiting {} seconds for next retry. Total timeout remaining: {}".format(
                        interval, int(timeout - (time.time() - t1))
                    )
                )
            else:
            ...
            raise Exception(
                "Timed-out waiting for analyis status to reach terminal state (analyzed or analysis_failed)"
            )
    ...
    try:
        ...
        if dockerfile:
            with open(dockerfile, "r") as FH:
                dockerfile_contents = base64.b64encode(
        ...
        if itype == "tag":
            ...
            ret = anchorecli.clients.apiexternal.add_image(
                config,
                tag=input_image,
                force=force,
...
def import_image(infile):
    ...
    try:
        ...
        ret = anchorecli.clients.apiexternal.import_image(
            config, anchore_data=anchore_data
        )
        ecode = anchorecli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                anchorecli.cli.utils.format_output(
                    config, "image_import", {}, ret["payload"]
                )
            )
        else:
            raise Exception(json.dumps(ret["error"], indent=4))
...
def get(input_image, show_history):
    ...
    try:
        ...
        if itype == "tag":
            ret = anchorecli.clients.apiexternal.get_image(
                config, tag=image, history=show_history
            )
        elif itype == "imageid":
            ret = anchorecli.clients.apiexternal.get_image(
                config, image_id=image, history=False
...
def query_content(input_image, content_type):
    ...
    try:
        ...
        if not imageDigest:
            ...
            ret = anchorecli.clients.apiexternal.query_image(
                config,
            ...
            if ret:
                if ret["success"]:
                    output = anchorecli.cli.utils.format_output(
                        config,
                        "image_content",
                        {"query_type": content_type},
                        ret["payload"],
                    ...
                    if not output:
                        raise Exception(
                            "There is no image content (%s) to provide for %s"
        ...
        print(
            anchorecli.cli.utils.format_error_output(config, "image_content", {}, err)
        )
        if not ecode:
            ecode = 2
...
def query_metadata(input_image, metadata_type):
    ...
    try:
        ...
        print(
            anchorecli.cli.utils.format_error_output(config, "image_metadata", {}, err)
        )
        if not ecode:
            ecode = 2
...
def delete(input_image, force, all):
    ...
    if all:
        try:
            ...
            for image in ret["payload"]:
                if image["imageDigest"]:
                    ret = anchorecli.clients.apiexternal.delete_image(
                        config, imageDigest=image["imageDigest"], force=force
                    )
                    if ret["success"]:
        ...
        try:
            ...
            if imageDigest:
                ret = anchorecli.clients.apiexternal.delete_image(
                    config, imageDigest=imageDigest, force=force
                )
                ecode = anchorecli.cli.utils.get_ecode(ret)
            else:
                ecode = 1
